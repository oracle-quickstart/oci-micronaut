...
BEGIN
	...
	BEGIN
		...
		INTO profileExists 
		FROM DBA_PROFILES 
		WHERE PROFILE = 'MUSHOP_ORDER_SERVICES';
		DBMS_OUTPUT.PUT_LINE ('** Profile creation steps - &_DATE');
		IF profileExists = 0 THEN
			DBMS_OUTPUT.PUT_LINE ('Creating Profile = MUSHOP_ORDER_SERVICES ...');
			EXECUTE IMMEDIATE
				'CREATE PROFILE MUSHOP_ORDER_SERVICES
		...
		ELSE
			DBMS_OUTPUT.PUT_LINE ('Profile = MUSHOP_ORDER_SERVICES exists, steps ignored');
		END IF;
	EXCEPTION
		WHEN OTHERS THEN
	...
	BEGIN
		...
		IF schemaUserExists = 0 THEN
			...
			EXECUTE IMMEDIATE 'CREATE USER &1 IDENTIFIED BY &2 PROFILE mushop_order_services';
